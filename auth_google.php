...
try {
    ...
    $returnVariables = $openid->getAttributes();
  	...
  	//set user email & first name variables. Don't so much care for last names. eventually need a link to avatar
	...
	$isuser->execute();
  	...
  	if (!$user_id){
		...
		// set session id & direct to home.php
  		//$_SESSION['user_id'] = $getid->fetchColumn();
  		
  		//$_SESSION['user_name'] = $getid->fetchColumn();
  		//$_SESSION['user_avatar'] = $getid->fetchColumn();
